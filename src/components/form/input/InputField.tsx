// InputField.tsx
import { forwardRef, type InputHTMLAttributes } from "react";

type InputProps = Omit<InputHTMLAttributes<HTMLInputElement>, "className"> & {
  className?: string;
  success?: boolean;
  error?: boolean;
  hint?: string;
};

const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    {
      className = "",
      success = false,
      error = false,
      hint,
      disabled = false,
      type = "text",
      ...rest // ← aquí llegan: id, name, placeholder, value, onChange, required, autoComplete, min, max, step, etc.
    },
    ref
  ) => {
    let inputClasses =
      "h-11 w-full rounded-lg border appearance-none px-4 py-2.5 text-sm shadow-theme-xs placeholder:text-gray-400 focus:outline-hidden focus:ring-3 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 " +
      className;

    if (disabled) {
      inputClasses +=
        " text-gray-500 border-gray-300 bg-gray-100 cursor-not-allowed opacity-40 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700";
    } else if (error) {
      inputClasses +=
        " border-error-500 focus:border-error-300 focus:ring-error-500/20 dark:text-error-400 dark:border-error-500 dark:focus:border-error-800";
    } else if (success) {
      inputClasses +=
        " border-success-500 focus:border-success-300 focus:ring-success-500/20 dark:text-success-400 dark:border-success-500 dark:focus:border-success-800";
    } else {
      inputClasses +=
        " bg-transparent text-gray-800 border-gray-300 focus:border-brand-300 focus:ring-brand-500/20 dark:border-gray-700 dark:text-white/90 dark:focus:border-brand-800";
    }

    const hintId = hint ? `${rest.id || rest.name || "input"}-hint` : undefined;

    return (
      <div className="relative">
        <input
          ref={ref}
          type={type}
          disabled={disabled}
          className={inputClasses}
          aria-invalid={error || undefined}
          aria-describedby={hintId}
          {...rest} // ← pasa todas las props nativas: required, autoComplete, value, onChange, min, max, step, etc.
        />

        {hint && (
          <p
            id={hintId}
            className={`mt-1.5 text-xs ${
              error ? "text-error-500" : success ? "text-success-500" : "text-gray-500"
            }`}
          >
            {hint}
          </p>
        )}
      </div>
    );
  }
);

Input.displayName = "Input";
export default Input;
