...
// Tipo de valores que maneja el formulario (centro o subcentro)
...
type CentroFormInitial = Partial<CentroFormValues>;
...
export default function CentroFormModal({
  ...
  initial,
  onSave,
  entityLabel = "centro",      // 👉 "centro" | "subcentro" (o lo que quieras mostrar)
  showDescripcion = true,      // 👉 para subcentros usualmente false
}: {
  open: boolean;
  onClose: () => void;
  initial?: CentroFormInitial;
  onSave: (values: CentroFormValues) => Promise<void> | void;
  entityLabel?: string;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await onSave({
        ...
        descripcion: (showDescripcion ? descripcion.trim() : "") || undefined,
        activo,
      });
    } catch (e: any) {
      setErr(e?.message || `No se pudo guardar el ${entityLabel}.`);
    } finally {
      setSaving(false);
    }
  }
